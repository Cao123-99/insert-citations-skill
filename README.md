...
学术论文参考文献处理工具——为 .docx 论文**查找真实文献**、**按出现顺序排序**、**插入 Word/WPS 交叉引用域**。三步走通一条龙，兼容 Word 和 WPS。
...
4. **始终执行 WebSearch**（即使文档已有文献），检索支撑断言的真实论文
5. 写入引用标记和参考文献列表（复用原标题、清除旧条目）
   - 状态 C：旧文献与新检索结果合并去重（标题相似度 + 第一作者匹配）
...
这些不是完整模式，而是单一操作，可直接对号入座：

| 你想做的 | 说的话/命令 | 执行顺序 |
|---------|------------|---------|
| 只排序不插域（已有引用和文献） | "按出现顺序重新整理参考文献" | 重排文献 → 自动编号 → 更新 REF + `[n]` 编号 → 停 |
| 排序+交叉引用（已有文献只排版） | `insert_citations.py paper.docx` | 重排 → 去手动编号 → 自动编号 → 更新编号 → 书签 → `[n]`→REF |
| 清除所有标记做干净排序 | `--strip-markers --sort-only` | 清 `[n]` + 清 REF → 重排 → 自动编号 |
| 只看报告不改文件 | "先看看这篇论文的引用情况" | `--dry-run` |
| 清除孤儿标记 | `--strip-markers` | 清纯文本 `[n]` + 清 REF 域 |
...
详见 [`FORMATS.md`](./FORMATS.md)，覆盖：

| 标准 | 适用场景 |
|------|---------|
| GB/T 7714-2015 | 中文论文最常用 |
| IEEE | 英文工程论文 |
| APA 7th | 社会科学 |

支持的文献类型：期刊 `[J]`、会议 `[C]`、专著 `[M]`、学位论文 `[D]`、在线资源 `[EB/OL]`、专利 `[P]`、预印本 `[J/OL]` 等。
...
2. 在 Claude 对话中把 `.docx` 文件拖进来
3. 说出触发语，例如"帮我全流程处理参考文献"
4. 按 Claude 提示确认格式偏好和插入位置
...
└── scripts/
    └── insert_citations.py   # 核心脚本（~1500 行）
```
